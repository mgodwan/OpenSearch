...
final class DVSkipConsumer extends DocValuesConsumer {
    ...
    private void writeTermsIndex(SortedSetDocValues values) throws IOException {
        ...
        long numBlocks = 1L + ((size + Lucene90DocValuesFormat.TERMS_DICT_REVERSE_INDEX_MASK)
        ...
        try (ByteBuffersIndexOutput addressOutput = new ByteBuffersIndexOutput(addressBuffer, "temp", "temp")) {
            ...
            for (BytesRef term = iterator.next(); term != null; term = iterator.next()) {
                if ((ord & Lucene90DocValuesFormat.TERMS_DICT_REVERSE_INDEX_MASK) == 0) {
                    ...
                    & Lucene90DocValuesFormat.TERMS_DICT_REVERSE_INDEX_MASK) == Lucene90DocValuesFormat.TERMS_DICT_REVERSE_INDEX_MASK) {
                        previous.copyBytes(term);
                    }
                ++ord;
            }
